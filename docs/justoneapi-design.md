...
## 1. 接入定位：构建期（**已交付**）+ 运行时（**已接入，见 §14 与节点设计**）

| | 构建期 | 运行时 |
| --- | --- | --- |
| 状态 | **已交付**（`npm run catalog:build:justoneapi`，见 §12） | **已接入**：图内条件触发节点 `enrichLiveData`（2026-09-25 落地） |
| 触发 | 人工执行 | 图里的条件边按需触发（token 已配置 / 结果非空 / 命中实时性关键词 / **id 可解析为平台 id** / 距上次 > 60 秒 / 未熔断） |
| 产物 | `data/justoneapi-catalog.json`（独立文件，**不覆盖** `real-catalog.json`） | 无产物，只写状态字段 `liveOverrides` / `liveFetchedAt` |
| 调用量 | 一次构建 = 每品类 2 次搜索（7 品类共 14 次）+ 每件保留商品的 1 次详情 | 单轮最多 3 件：默认 1 次价格调用/件；命中库存关键词再 +1 次详情调用/件（同一商品 5 分钟内走缓存 + 单飞） |
...
**运行时形态由 §14 裁定（v2）**：不做图之外的独立函数，改成图里的**条件触发补充节点** `enrichLiveData`；执行顺序也按 §15.3 的裁定调整为「先构建期源、再节点接入」（默认 `real` 源下商品 id 全是 Amazon ASIN，节点没有可拉取的目标）。节点侧的两处收窄见 §14.4 的修订说明。
...
> **2026-09-25 落地时的两处修订（以代码为准）**
>
> 1. **覆盖范围收窄为 `price` + `stock`**：构建期实测已证明京东搜索与详情端点**没有** `rating` / `reviews` / `sales`（空串或「1万+」区间文案），因此原计划的「覆盖评分/评论数」取消——不是漏做，是上游没这些字段。
> 2. **按需取库存**：价格端点（`/api/jd/get-item-price/v1`）只返回 `{good_id, price}`（单位分），不含库存。因此默认只调价格端点（≤3 件 → ≤3 次计费）；**仅当**用户本轮命中库存类关键词（还有货 / 库存 / 有货 / 缺货 / 现货）时才额外取详情端点读 `StockState`（每件 +1 次）。两种模式都受「单轮最多 3 件」约束。
>
> 另有一条一致性规则：价格端点不给划线价，所以只覆盖 `price`；若覆盖后原 `originalPrice` 不再大于新价，则把划线价同步为新价格（避免「划线价低于现价」的破图）。
