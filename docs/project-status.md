...
> - 版本锚点：git 仓库 `https://github.com/Zeffy-Real/eshopagent`，本次同步前 HEAD 为 `561e772`（阶段 19 + 彩排补充轮）；本次同步内容为**演示彩排与补充轮结果**（2026-09-25，见 [demo-rehearsal.md](./demo-rehearsal.md)）
> - 校验状态：`tsc --noEmit` 0 错误；**327 个单测全绿（21 个文件）**；`next build` 通过（首页 334 kB / First Load 470 kB）
...
| 测试 | **327 个单测用例**（Vitest，21 个文件，覆盖口径一致性的唯一实现、记忆机制、会话持久化、落地校验重试、在途请求中止、JustOneAPI 码表与字段映射、实时补充节点六条判定与静默失败、条件串去重与 LLM 失败文本清洗）；无组件/E2E 自动化测试 |
