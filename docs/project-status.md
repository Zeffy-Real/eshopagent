...
> - 版本锚点：git 仓库 `https://github.com/Zeffy-Real/eshopagent`，本次同步前 HEAD 为 `fb70cf7`（阶段 19）；本次同步内容为**演示彩排结果**（2026-09-25，见 [demo-rehearsal.md](./demo-rehearsal.md)）
> - 校验状态：`tsc --noEmit` 0 错误；**321 个单测全绿（21 个文件）**；`next build` 通过（首页 334 kB / First Load 470 kB）
...
| 源码规模 | **119 个 `.ts` / `.tsx` 文件**（`app` / `components` / `lib` / `store` / `scripts`，其中 21 个单测文件）+ 9 个 `.mjs` 与 6 个 `.d.mts`（京东实时源；`.mjs + .d.mts` 的原因见设计文档 §15.6） |
...
| ~~前端「实时」标注的视觉~~ | ✅ **已补验（2026-09-25）**：浏览器截图 + DOM 断言三项齐全（卡片标注 3 处、hover 说明、时间线条目），实证见 §7.1 与 [demo-rehearsal.md](./demo-rehearsal.md)。**仍受限**：彩排浏览器视口被固定为窄屏 444×559，**没有桌面三栏（≥1280px）视口下的标注截图** —— 标注与时间线在 Tab/抽屉布局下逐项可见 |
...
| 19 | **条件串可能重复一个词**（2026-09-25 彩排实测 `图书 · 小说 · 小说`） | 展示层 | `describeFilters`（`lib/agent/ruleParser.ts`）按 品类 → keywords → tags 拼接，**不做跨列表去重**；同一 thread 混合两种解析来源时（LLM 把题材词写进 keywords、规则解析归 tags），同一个词会同时出现在两份列表里。只影响时间线/回复里的条件文案，**不影响检索结果**（命中件数一致）。本轮约束「不为演示更好看改业务逻辑」，**只记录不修** |
| 20 | **无 Key 时意图面板字段名与 LLM 路径不一致**（「功能 小说」vs「关键词 小说」） | 展示层 | 两种解析器对同一题材词的归类不同（tags vs keywords），面板按各自字段渲染。彩排发现，可能被审查者追问，**只记录不修** |
| 21 | **LLM 失败时时间线原样显示服务商错误文本**（含服务商掩码后的密钥末位） | 安全（低） | `parseIntent` 把 `llmError` 截 80 字写进时间线（`lib/agent/nodes/parseIntent.ts`）：好处是现场一眼看出失败原因，代价是密钥**末 4 位**会随错误文本出现在界面上（由服务商掩码，非明文；实测用测试无效值，未涉及真实密钥）。若要收紧，可对错误文本做一次清洗（去掉 `api key` 相关片段）后再写入 |
