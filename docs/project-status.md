...
| 测试 | **84 个单测用例**（Vitest，7 个文件，覆盖口径一致性的唯一实现）；无组件/E2E 自动化测试 |
...
| 单测 | `npm test`（Vitest，7 个文件） | ✅ 84 / 84 通过 |
...
| 4 | **MemorySaver 无上限** | 稳定性 | 阶段 13 计划换 SqliteSaver + 惰性会话清理 |
...
| 7 | ~~无测试、非 git 仓库~~ | 工程化 | ✅ **已修（阶段 11.5）**：84 个 Vitest 单测 + git 仓库（远端 `Zeffy-Real/eshopagent`） |
