...
> - 版本锚点：git 仓库 `https://github.com/Zeffy-Real/eshopagent`，本次同步前 HEAD 为 `ea15792`；**解冻轮已收尾、重新冻结**（2026-09-26）：中栏空结果态（三态修正）+ 重新走通 A / C 两阶段演示脚本，发现项见 §8 第 31–33 条
> - 校验状态：`tsc --noEmit` 0 错误；**385 个单测全绿（27 个文件）**；`next build` 通过（首页 288 kB / First Load 425 kB；上一轮 288 / 424）
...
| 32 | **检索条件的「粘性」**：某轮设过的字段会被后续搜索继承 | 真实缺陷（**待裁定，本轮未修**） | 现象：`searchFilters` 是**浅合并** reducer（refine 只改一个字段的既定语义），而 `parseIntent` 的 `toFilters` 只写「本轮解析出来的字段」、**不把未提到的字段置空** → 一旦某轮设过 `minPrice`（实测「十万元以上的商品」→ `minPrice=100000`），后续**新的搜索**若没重新给出该字段就会继承它（实测后续「推荐几本小说」仍带 ¥100000 下限 → 命中 0、中栏停在空结果态）。成本低但**触及筛选语义**（要区分 search 与 refine 两条路径：search 时重置未提及字段、refine 保留），按本轮「不逐个修 §8」的约束未动；演示动作（`demo-script.md` 加分动作第 4 条）已附「演完点开启新会话清条件」的提醒 |
