...
### 6.8 看全部 / 浏览视图（2026-09-26 浏览视图轮）

| 主题 | 文件 | 变更 |
| --- | --- | --- |
| 清空判据 | `lib/agent/ruleParser.ts` | 新增 `detectClearAll`（强 / 弱触发 + 排除词 + 「原话未点名」判据）+ 触发词伪关键词剔除（`isTriggerVocabulary`）；`guessIntentByRules` 最前接入（命中强制 `search`） |
| 条件清空 | `lib/agent/nodes/parseIntent.ts` | 新增 `clearAllFilters`（八字段显式 undefined）、清空分支（只看原话 + `intent ∈ {search, refine}` 门挡）、时间线 `clear_filters` 记录、每轮写 `refineCount: 0` |
| 计数与区间 | `lib/agent/nodes/refineSearch.ts` / `lib/agent/ruleParser.ts` | 文案改「本轮第 N/2 轮调整条件」；`relaxFilters` 返回前 clamp 反向价格区间 |
| 浏览视图（C） | `components/product/browse-view.tsx`（新增）/ `lib/catalog/browse-products.ts`（新增）/ `category-bar.tsx` / `product-panel.tsx` / `store/use-ui-store.ts` | 中栏覆盖层：三来源（品类 / 全量 / id）、分页加载更多、复用 `SortControl`（`sortProducts` 从面板提到控件旁，一份实现）、ESC + 关闭按钮；chip 改为直接开视图；`React.lazy` 按需挂载 |
| 「查看全部 N 件」（A） | `lib/agent/{events,state,sse}.ts` / `lib/agent/nodes/searchProducts.ts` / `lib/agent/tools/productTools.ts` / `components/common/panel-header.tsx` | 新增只读字段 `searchResultIds`（上限 120、仅有筛选且命中多于展示时写、快照按意图门挡）；`filterProducts` 返回全量命中 id；`PanelHeader` 新增 `subtitleAction` 插槽承载入口 |
| 共享判据 | `lib/product-panel-state.ts` | 新增 `hasEffectiveFilters`（服务端决定要不要下发 id、客户端决定走目录还是走 id，**同一实现**） |
| 文档 | README / `docs/{demo-script,project-status}.md` | README：分类浏览与浏览视图两行、展示口径与入口、state 字段表、首屏体积沿革、客户端数据流三处来源；demo-script：加分动作 5 改为「chip 零 LLM 依赖浏览全部」+ 新增动作 6「看全部」、第 8 步无 Key 说明；本文件：§6.8 / §7.1 新块 / §8-34 部分作废标注 / §8-36 补第三例 / §8-37-38 |

### 6.9 中栏就地展开（2026-09-26 展示集拆分轮，commit `a83fc75`）

**边界**：`searchResults`（12 件）是 **Agent 的工作集**，被 SSE 快照（每轮 4–5 帧）、`generateReply`、`grounding`、`compareProducts`、`selectReplyProductIds` 消费 —— 本轮**不动它、不改它的长度、不改 SSE 协议**。做法是把「展示集」从中拆出来：多出来的部分由**客户端按需解析**（与浏览视图同一个目录 chunk），首屏与 SSE 载荷都不变。

| 主题 | 文件 | 变更 |
| --- | --- | --- |
| 展示窗口（唯一实现） | `lib/product-panel-state.ts` | 新增 `EXPAND_PAGE_SIZE = 24`、`expansionPlanOf`（来源与上限：ids 源 ceiling = id 条数、all 源 ceiling = 命中数、两者都拿不到 → `null`）、`needsFullSetForSort`（排序前是否必须先载入完整集）、`expansionViewOf`（窗口 + 余量，`expanded === null` → 回落首屏 12 件 = 失败保留的纯逻辑）、`applyServerOrder`（客户端目录顺序 → 服务端结果顺序，前 12 件不变样的前提）；`sortProducts` 从 `components/product/sort-control.tsx` 移入（避免 lib→component 反向依赖，浏览视图与中栏同读一份） |
| 中栏 | `components/product/product-panel.tsx` | 删 `browseAllSource` memo 与「查看全部 N 件」入口；新增 `expanded` / `visible` / `expanding` / `expandError` 四态 + `resultKey`（本轮结果指纹：新一轮重置、SSE 帧之间不折叠）+ `roundRef`（在途加载回来看是否还算数）；网格底部「加载更多（已显示 M/N）」→ 到顶「已显示全部 N 件」/ 截断时「已显示前 120 件（单次上限 120 件）」；失败行「更多结果加载失败（· 排序需要完整结果集）」+ 重试（重试 = 继续那次展开）；排序 onChange 走 `needsFullSetForSort` 先载入再排，载入期间不重排（`awaitingFullSet` → 按服务端顺序显示，避免「只排了 12 件」的假象）；副标题的展示数改用当前渲染数 |
| 收口 | `components/common/panel-header.tsx` / `components/product/browse-view.tsx` / `lib/catalog/browse-products.ts` / `store/use-ui-store.ts` | `PanelHeader` 删 `subtitleAction` 槽位（回到原样）；浏览视图删 `ids` 分支与相关文案、分页步长改引 `EXPAND_PAGE_SIZE`；`BrowseSource` 保留 `ids`（就地展开要用），新增 `BrowseViewSource = Exclude<BrowseSource, {kind:'ids'}>` 供 `browseSource` / `openBrowse` 收窄 —— **职责变成：搜索结果就地展开归中栏，浏览视图 = 无对话时的目录浏览（点 chip / 看全量）** |
| 文档 | README / `docs/{demo-script,project-status}.md` | README：展示口径行（首屏 12 件 + 加载更多）、浏览视图行（两来源）、state 字段表 `searchResultIds` 消费者、首屏体积沿革、单测数；demo-script：加分动作 6 改写；本文件：§6.9 / §7.1 新块 / §8-38 收口标注 / §8-40 |
...
#### 2026-09-26 看全部 / 浏览视图轮（②③ + C + A）

| 项 | 方法 | 结果 |
| --- | --- | --- |
| ② 清空条件（SSE 直连，0 计费） | 脏会话「推荐几件数码的商品」→「让我看看所有420件商品」**连跑 8 次** | ✅ 8/8 通过（8 次 LLM 全判 `refine`）：条件清空为 `{rawQuery}`、命中 420、每次都有 `清空条件：重置全部筛选条件｜数码 → 全部商品` |
| — 正例变体与负例（10 条） | 「不限品类，看看全部商品」/「所有商品加起来多少钱」「平均评分」「全部加入购物车」「对比前 3 件」「看看所有小说」「换成第二件」「再便宜点」「推荐几本小说」「重置预算」「不限品牌」 | ✅ 变体清空到 420；统计类两句**不清空且条件原样保留**（闲聊轮）；cart / compare 被意图门挡；「看看所有小说」走 switch → `图书 · 小说` 30 件；指代 / 细化 / 新目标三种基线不变；「重置预算」「不限品牌」清空整份（**字段级重置不支持**，见 §8-37） |
| ③ refineCount 每轮重置 + clamp | 连续 3 轮「10 元以内的耳机」；「十万元以上的商品」→「再便宜点」×2 | ✅ 每轮都是「本轮第 1/2 → 2/2」（旧实现第 2 轮起显示「第 3/2」且不再放宽）；第二轮 clamp 成 `¥100000 - ¥100000`（旧实现产出 `¥100000 - ¥70140` 反向区间） |
| — 规则路径（无 Key） | 进程级空 Key 启动 3100：「看看所有商品」/「清空条件」/「不限品牌」/「把购物车清空」 | ✅ 前三条强判 search + 清空（时间线 `规则解析 · 全部商品` + `清空条件｜数码 → 全部商品`）；「把购物车清空」仍 cart 且不清空 |
| C 浏览视图（真机 1440×900，prod build） | 点「数码 60」chip → 加载更多 ×2 → 价格排序 → ESC / 退出浏览 | ✅ `数码 · 全部商品｜目录里该品类共 60 件`、24 → 60 件、「已显示全部 60 件」、排序生效、两种退出都可；**点击前脚本列表无浏览 / 目录 chunk，点击后两者出现**（按需下载）；无 Key 下同样可用（[round3-browse-all.jpg](./rehearsal/round3-browse-all.jpg)、[round3-nokey-clear.jpg](./rehearsal/round3-nokey-clear.jpg)） |
| A「查看全部 N 件」（SSE + 真机） | 「推荐几本小说」（30）/「看看数码的商品」（60）/「500 元以内的商品」（361）/「让我看看所有420件商品」（420）/「你好」/「换成第二件」 | ✅ 30 → 30 个 id、60 → 60 个 id、361 → **120 个 id（上限生效，视图注明「显示前 120 件（单次上限 120 件）」）**、420（无筛选条件）→ **0 个 id**（客户端目录现算，420 个 id 不进 SSE）、闲聊与序数轮 → 0 个 id 且**无入口**（[round3-view-all-entry.jpg](./rehearsal/round3-view-all-entry.jpg)） |
| — 载荷实测（每帧增量） | 解析状态帧的 `searchResultIds` JSON 体积 | ✅ 30 id = 0.47 KB/帧、60 id = 1.42 KB/帧、120 id = **2.52 KB/帧**（≤4 KB 预算；每轮 4–5 帧 → 120 id 约 +10～13 KB/轮）；对应末帧 10.0 / 13.0 / 13.3 KB |
| 首屏体积（前后对比） | 先停 dev → `npm run build`，C 后与 A 后各一次 | ✅ 基线 HEAD 288/425/103 → C 后 **289/425/103**（First Load 不增加：视图用 `React.lazy` 按需挂载，`next/dynamic` 会把运行时带进首屏、实测 +2 kB）→ A 后 **289/426/102**（+1 kB = 入口按钮与判据）；浏览视图 chunk 5.0 KB、目录 chunk 410.5 KB **都只在点开时下载** |
| 单测 | `ruleParser` / `parseIntent` / `refineSearch` / `searchProducts` / `sse` / `product-panel-state` / `browse-products` | ✅ **444 全绿（28 文件）**：词表正反例、清空四边界与 reducer 语义、refineCount 重置与 clamp、searchResultIds 三条门挡 + 意图门挡、`hasEffectiveFilters`、浏览三来源与分页窗口 |
| 全量校验 | `tsc` → `npm test` → `npm run build` | ✅ `tsc` 0 错误；build 通过（数字见上一行） |

#### 2026-09-26 快照恢复归一化（旧持久化快照白屏修复，§8-39）

| 项 | 方法 | 结果 |
| --- | --- | --- |
| 复现 ①（崩溃） | dev 3000 真机：把 `localStorage['eshop-agent'].state.snapshot.searchResultIds` 删掉 → 刷新 | ✅ 稳定复现：整页 `Application error: a client-side exception has occurred`（0 按钮 / 0 卡片），栈 `ProductPanel.useMemo[browseAllSource] components\product\product-panel.tsx (80:37)` |
| 复现 ②（静默错） | 只删 `searchTotal`（保留 ids） → 刷新 | ✅ 不崩但**静默错**：副标题写成 `12 件商品`（丢了「共 30 件 · 展示前 12 件」）、入口文案写成 `查看全部  件` |
| 验收（真机，4 条） | ① 旧快照（缺 ids + total）；② 旧快照后发一轮新检索；③ 完整快照刷新；④ 清掉整个 `eshop-agent` | ✅ ① 正常渲染（12 卡片 / 无控制台报错）、无 undefined 文案、**入口不出现**（ids 为空不猜值）；② 新检索后回到 `共 30 件 · 展示前 12 件 · 图书 · 小说` + 入口 `查看全部 30 件`；③ 刷新后结果与入口照旧，点入口 → `本轮命中 30 件`（浏览视图正常）；④ 回落「为你推荐 · 目录共 420 件 · 按评分与销量精选 12 件」 |
| 单测 | `lib/agent/events.test.ts`（新增 8 条） | ✅ 非对象 → null、缺 `searchResultIds` → `[]`（本次崩溃场景）、缺 `searchTotal` → `searchResults.length`、`false` / `0` / `''` / `null` 不被顶掉、完整快照 `toEqual` 原样、**字段清单断言**（从完整快照的键推导）；`tsc` 0；**452 全绿（29 文件）** |

#### 2026-09-26 中栏就地展开（展示集从工作集拆出，commit `a83fc75`）

| 项 | 方法 | 结果 |
| --- | --- | --- |
| 搜「食品」60 命中（真机 1440×900，dev） | 首屏 12 件 → 点「加载更多」×2 | ✅ 12 → 36 → 60；按钮文案 `加载更多（已显示 12/60）` → `（已显示 36/60）`；到顶 `已显示全部 60 件`；副标题随展示数走（`共 60 件 · 展示前 12 件` → `共 60 件 · 展示前 36 件` → `60 件商品 · 食品`）；**全程不跳视图、对话可见**（[round4-expand-60.jpg](./rehearsal/round4-expand-60.jpg)） |
| 搜「推荐几本小说」30 命中 + **排序前载入** | 只显示 12 件时**直接点「价格」** | ✅ 列表 12 件、价格升序 38 → 91；**首件 ¥38 不在原首屏 12 件里（原首屏最低 ¥44）** → 证明是「先载入完整 30 件再排」而不是只排那 12 件；再点「加载更多」→ 30 件全升序（38 → 180）+ `已显示全部 30 件`；切回「综合」→ 回到服务端顺序、前 4 件与首屏逐字一致 |
| 无筛选条件（命中 420） | 「让我看看所有420件商品」→ 连续「加载更多」 | ✅ 17 次点击 → 420 件 + `已显示全部 420 件`；**展开后的首 4 件（含价格 646 / 13 / 693）与展开前逐件一致**（`applyServerOrder` 对齐生效：目录原始顺序会在这里露馅）；快照 `searchResultIds` 为空（420 个 id 不进 SSE） |
| 有筛选且命中 > 120 | 「500元以内的商品」（命中 361） | ✅ 按钮分母即上限：`加载更多（已显示 12/120）`；展开到 120 → `已显示前 120 件（单次上限 120 件）`、副标题 `共 361 件 · 展示前 120 件 · ¥500 以内`；快照里 `searchResultIds = 120` 而 `searchResults = 12`（**工作集未变**） |
| 坏 chunk 注入（0 计费） | 隐藏目录 chunk 文件 → 刷新页面（清空模块注册表）→ 点「加载更多」 | ✅ 12 件**原样保留**（列表逐件未变）、只多一行 `更多结果加载失败` + 「重试」；无 `nextjs-portal` 错误覆盖层、页面不崩；恢复 chunk 后点「重试」→ 36 件（**重试 = 继续那次展开**，证据图 `.cache/diag/expand-failure.png`） |
| 闲聊轮 / 序数轮 | 「你好呀」/「换成第二件」 | ✅ 两轮都**没有**「加载更多」、也没有失败行（`expansionPlanOf` 按意图门挡）；闲聊轮收起回 12 件、序数轮 1 件 |
| 载荷（口径与改前一致） | `.cache/diag/run-expand-cases.mjs` 直连 `/api/agent` 解析状态帧 | ✅ 食品（60 id）末帧 11.3 KB / ids 1.02 KB；420（无筛选）**0 个 id**、末帧 11.1 KB；361 命中 → **120 id = 2.52 KB/帧**（≤4 KB 预算）；本轮改动全在客户端（服务端文件零改动）→ **SSE 协议与字段口径未动** |
| 首屏体积（前后对比） | 先停 dev → `npm run build` | ✅ HEAD **289 / 426 / 102** → 本轮 **290 / 427 / 102**（+1 kB = 收尾行与判据）；16 个首屏 chunk 里**搜不到任何商品 id**；目录 chunk 410.5 KB、浏览视图 chunk **4.5 KB**（删 ids 分支后 −0.5 KB）仍只在点开 / 展开时下载 |
| 单测 | `lib/product-panel-state.test.ts`（新增 17 条） | ✅ 追加窗口与余量、来源选择（ids / all / 都没有）、`needsFullSetForSort` 四边界、**失败保留**（`expanded === null` 回落首屏）、排序作用于完整集（首件是全局最低价）、`applyServerOrder` 与服务端 `filterProducts` 的 **parity**（relevance + 四个显式排序键逐件比对）、ids 源前 12 件与首屏逐件一致、`EXPAND_PAGE_SIZE = 24` |
| 全量校验 | `tsc` → `npm test` → `npm run build` | ✅ `tsc` 0 错误；**469 全绿（29 文件）**；build 通过（数字见上一行） |
...
| 34 | ~~无 Key 时点品类 chip 不检索~~（规则路径下「帮我看看服饰的商品」判 chat、条件被清、回通用引导） | 功能缺口 | ✅ **已修（2026-09-26，与第 32 条同轮，commit `d646ea2`）**：根因是规则解析器的品类词表**漏了 4 个品类名**（服饰 / 食品 / 家居 / 美妆 —— 运动 / 数码 / 图书此前已有），且「帮我看看<品类>的商品」不命中任何 search 触发词。修法只补 7 个品类名本身（**不扩充通用词表**，避免改变其他意图判定）+ 品类名触发 search 意图（放在规则列表最后，让对比 / 结算 / 加购 / 细化优先命中）。实测（LLM 关）：chip 消息变为 `search · 服饰 · 60 件`；`你好` 仍 chat、`推荐几本小说` 仍 search（规则路径「小说」进 tags，字段名差异见第 20 条）、`推荐几件数码的商品` 仍 60 件。<br/>⚠️ **部分作废（2026-09-26 浏览视图轮）**：品类 chip 改为**直接打开中栏浏览视图**（不进对话、零 LLM 依赖），chip 不再经过意图解析，因此本条修复只对「用户在对话里说『帮我看看<品类>的商品』」这条路径继续有效；chip 也不会再触发任何条件继承 / 重置（它根本不写筛选条件） |
...
| 36 | **规则路径的两处条件污染**（① 题材词进 tags 导致类别残留；② 序数词被抽成关键词） | 真实缺陷（成本高，记录不修） | ① 先点「服饰」chip（或任何带品类的轮次）后说「推荐几本小说」→ 规则解析把「小说」归入 `tags`（第 20 条的桶语义），而「切换浏览目标」判据只认 `category` / `keywords` → 不触发重置，检索「服饰 · 小说」0 命中后被自动放宽丢掉「小说」→ 回到服饰全量（用户要小说拿到服饰）。LLM 路径正常（「小说」进 `keywords` → 30 件）。② 「把第一件加入购物车」在规则路径下抽词抽到「第一」（目录商品名里的字面片段）→ 条件串被写成「第一」并顶掉上一轮条件（加购本身正常、由序号定位，检索结果不受影响）。两处都**已确认修复前后行为一致（非本轮引入）**；①的修法要动规则解析的桶语义（第 20 条明确不改）或给判据引入「题材词」名单（词表扩展被禁止），②的修法要收紧关键词抽取（会波及其他输入）→ 只记录（[demo-rehearsal.md](./demo-rehearsal.md) 11.5 F3 / F4）。<br/>**同类第三例已修（2026-09-26 浏览视图轮，见第 37 条）**：目录里两个商品名含「所有」（「适用于所有车辆」「适合所有肤质」），n-gram 把「所有」抽成关键词 —— 触发词自身不再算关键词；①②仍保留 |
| 37 | ~~**「看全部」落在判据缝隙里**：脏会话里说「让我看看所有 420 件商品」，上一轮的「数码」照样残留~~（旧判据只有「新目标 → 重置细分」与「否则 → 全量继承」两支；实测 LLM 完全不回写时条件照样残留） | 真实缺陷 | ✅ **已修（2026-09-26 浏览视图轮）**：新增 `detectClearAll`（`lib/agent/ruleParser.ts` 一处词表）：**强触发**（清空条件 / 重置 / 从头开始 / 重新开始 / 不限 / 随便看看 / 随便逛逛 / 都行）∨**弱触发**（浏览动词 ∧ 聚合词「全部 / 所有 / 整个目录 / 3–4 位数字+件」），且**无排除词**（加起来 / 多少钱 / 总价 / 一共 / 合计 / 平均 / 统计 / 共多少 / 几件）、**原话未点名任何东西**（`extractFiltersByRules` 的 category / keywords / tags / brands 全空）。**只看用户原话**，不读 LLM 输出、不读 filters —— 这是修「拿 LLM 回写的旧值当本轮证据」的关键。命中且 `intent ∈ {search, refine}` → `clearAllFilters`（八个字段**显式 undefined**，浅合并 reducer 下才真覆盖；`sort` 作为展示偏好保留）+ 时间线记录 `清空条件：重置全部筛选条件｜… → 全部商品`。**规则路径**在 `guessIntentByRules` 最前接入同一判据（命中强制 `search`，「清空条件」不会再被加购词表的「清空」抢走）。**已知限制**：① **字段级重置不支持** ——「重置预算」「不限品牌」清空的是整份条件（单命中、行为可预期、且有可见记录）；② 中文数字（「四百二十件」）不命中；③ LLM 把清空类表达判成 `chat` 时不清空（意图门挡的代价，8 连跑未出现）。**实测**：脏会话连跑 **8/8** 通过、命中 420、每次都有清空记录；10 条负例全部保持原行为（统计类不清空、cart / compare 被门挡、「看看所有小说」走 switch） |
| 38 | **展示上限 12 没有出口**：中栏写「共 N 件 · 展示前 12 件」，但没有任何「看更多」的入口（用户无法自由查看商品） | 产品缺口 | ✅ **已实现（2026-09-26 浏览视图轮）**：**C（品类 chip → 浏览视图）**：点 chip 直接打开中栏覆盖层（**同一组件三种数据来源**：品类 / 全量 / 命中 id），分页「加载更多」、排序复用现有控件、关闭按钮 + ESC、窄屏全宽；数据走客户端目录的**按需 chunk**（`lib/catalog/browse-products.ts` 独立模块 + `React.lazy`），**零 LLM 依赖**（无 Key 下实测可用）。**A（「查看全部 N 件」）**：新增只读字段 `searchResultIds`（`searchProducts` 写、`toSnapshot` 按意图门挡），上限 `SEARCH_RESULT_ID_LIMIT = 120`（实测 120 id ≈ 2.52 KB/帧 ≤ 4 KB 预算）；**无筛选条件时不下发 id**（客户端目录现算，420 个 id 不进 SSE），来源判据 `hasEffectiveFilters` 服务端与客户端**同一实现**。超限时视图注明「显示前 120 件（单次上限 120 件）」。**首屏**：基线 425 kB → C 后 425 kB（不增加）→ A 后 426 kB（+1 kB 入口按钮与判据）；视图 chunk 5.0 KB、目录 chunk 410.5 KB 均为点开才下载。**明确不做**：无限滚动、字段级重置、独立路由的商品详情页。<br/>⚠️ **部分作废（2026-09-26 展示集拆分轮，commit `a83fc75`）**：A 的「查看全部 N 件」入口（含 `PanelHeader.subtitleAction` 槽位）与浏览视图的 `ids` 分支已删 —— 搜索结果的展开改为**中栏就地「加载更多」**（见第 40 条），`searchResultIds` 字段**保留**（就地展开的数据源，下发口径不变：仅搜索轮、上限 120、无筛选不发）。C 的 chip 入口与「看全部 420 件」不受影响（浏览视图现在只承载目录浏览两条来源） |
| 39 | **旧持久化快照缺新字段 → 整页白屏**（`snapshot` 随 store 进 localStorage，而 zustand rehydrate 是浅合并、无形状校验 —— 字段是逐步长出来的） | 真实缺陷 | ✅ **已修（2026-09-26）**：真机复现两例 —— ① 删掉 `searchResultIds` 再刷新 → `product-panel.tsx:80` 的 `.length` 抛错、整页 `Application error`（栈：`ProductPanel.useMemo[browseAllSource]`）；② 只删 `searchTotal` → 不崩但**静默错**：中栏退化成「12 件商品」、入口写成「查看全部  件」。修法：新增纯函数 `normalizeSnapshot`（放在 `lib/agent/events.ts`，与 `AgentStateSnapshot` 同处，让「接口 / 归一化」的改动在同一屏内完成）并挂到 persist 的 **`merge`**（恢复的唯一入口）：非对象 → `null`（视为没有快照）；缺失字段补安全默认值（`searchResultIds` / `replyProductIds` → `[]`、`searchTotal` → **`searchResults.length`**、五个集合类 → 空集合、`pendingOrder` / `comparison` → `null`、`llmEnabled` → `true` …… 逐条理由写在函数注释里），已有值原样保留（`0` / `false` / `''` / `null` 不被默认值顶掉）；类型不开洞（字段仍必填），接口处加约定注释「新增字段必须同步补默认值」，单测含**从完整快照的键推导出的「字段清单」断言**（下次加字段忘补默认值会直接失败）。**同族隐患（本轮只列不改）**：`eshop-cart.items[].quantity`（缺失 → 合计 NaN）、`eshop-orders.orders[].items` / `status`（`order.items.length` 会抛错）、`eshop-agent.userProfile.recentSearches`（`profile-section` 直接读 `.length`）—— 三处同样是「旧形状 → 新消费」，需要时按同一模式各自补归一化即可 |
| 40 | **中栏就地展开的三条已知行为**（换轮 / 刷新收起；排序首次多一次目录加载；420 件要 17 次点击） | 已知行为（设计） | ① **按「轮」重置**：`expanded` / `visible` 是面板组件状态（不进 store、不进 localStorage），刷新页面或**任意新一轮**（含闲聊轮）到达都会收起回首屏 12 件——展开口径属于「本轮结果」，而闲聊轮的 `searchTotal` 是历史值，不该让它继续决定「还能展开多少」。② **排序的一次加载延迟**：完整集未载入时点非「综合」排序，会先解析目录 chunk（本地 <100 ms，慢网络更久）；期间列表保持服务端顺序（不给「只排了 12 件」的错顺序），按钮显示「正在加载…」；失败则保留 12 件 + `更多结果加载失败 · 排序需要完整结果集` + 重试。③ **点击次数**：420 件从 12 展开到顶需 17 次（每次 +24）——按既定约束「不做无限滚动、保持点一次多一屏的可预期性」，这是预期行为（commit `a83fc75`） |
