...
2. **长任务不该放进请求路径** —— 构建要下载数据集（数 MB）、跑归一化与去重，本地化还要几十次 LLM 调用（实测 420 件 ≈ 70 批、约 3.7 分钟）。把它塞进 Next.js 的请求里会撞上超时、无法中断、失败留半成品。
...
然后重启 dev。面板里给出的就是这条命令，但它**不执行**任何东西。
...
**本地化是增量的**：`catalog:localize` 会跳过已带 `nameOriginal` 的条目，只翻新增商品（失败批次下次运行自动补上）。⚠️ 但**构建会丢掉上一轮的本地化结果**（中文文案与 `nameOriginal` 都是 localize 写的，重新 build 就没了）——所以「先 build 再 localize」这条完整流水线第一次跑仍是全量翻译，增量只在「同一份产物上重复跑 localize」时生效；保住已验收文案的做法见 `docs/project-status.md` §7.1。
