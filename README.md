...
npm test           # 纯函数单测（150 个用例，覆盖口径一致性的唯一实现、记忆机制与会话持久化）
npm run build      # 生产构建
```
...
4. **页面关闭 / 刷新时不中止在途请求**：会话身份变更（清除画像 / 清空对话 / 新会话）已经会 `abort()` 在途请求并静默收尾（不弹错误、不把旧 token 写进新列表）；但关页面、刷新、浏览器导航仍会把服务端那一轮跑完（无害但浪费 token）。
...
8. **控制台偶发 `net::ERR_ABORTED`**：在途 SSE 请求被**页面刷新 / 导航**打断时浏览器会记一条 `net::ERR_ABORTED`（属浏览器行为，不影响功能）。主动中止的三个场景（清除画像 / 清空对话 / 开启新会话）已不再产生这条噪声 —— store 统一用 `AbortController` 中止，并在 catch 里区分「主动中止」与网络错误（前者静默收尾）。剩余场景需要在卸载时机上再补一次 abort。此外，销量覆盖率有限（112 件里 17 件有真实销量），若希望卡片普遍显示销量，应接入带销量字段的数据源。
