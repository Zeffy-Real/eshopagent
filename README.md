...
lib/catalog/products.ts  ── 按需 import() ──► 中栏浏览视图（品类 / 全量 / 命中 id 三来源，独立 chunk，点开才下载）
```

- **为什么这样切**：客户端原先静态 import 目录（`category-bar` / `product-panel` / `catalog-section` / `message-bubble` 四处），整份目录被内联进首屏 bundle——实测 First Load JS 555 kB，目录占约 **127 kB**。改完之后 **555 → 423 kB**（首屏 10 个 chunk 里搜不到任何商品 id，目录只出现在按需 chunk 里；此后 2026-09-26 新增购物车/订单抽屉与中栏三态：423 → **425 kB**，多出的 2 kB 是订单 store、抽屉组件与三态判定；同日新增浏览视图与「查看全部 N 件」入口：425 → **426 kB**，多出的 1 kB 是入口按钮与判据本身——浏览视图组件（5.0 KB chunk）与目录（410.5 KB chunk）都只在点开时下载）。
...
npm test           # 单测（385 个用例：口径一致性的唯一实现、记忆机制、落地校验重试、在途请求中止、JustOneAPI 码表与映射、实时补充节点六条判定、检索命中总数与展示口径、中栏三态判定、订单历史合并规则）
npm run build      # 生产构建
```
...
| 浏览视图（看全部） | 中栏覆盖层，**同一组件三种数据来源**：品类全量（chip）/ 整个目录（「让我看看所有 420 件商品」这类无筛选条件的命中）/ 本轮命中 id 列表（有筛选条件时，副标题旁的**「查看全部 N 件」**入口）。单次下发 id 上限 120（超出时视图注明「显示前 120 件」），无筛选条件的大集合走客户端目录现算、**420 个 id 不进 SSE**；视图代码与目录都在按需 chunk 里，首屏只多一个入口按钮 |
...
| 商品展示 | 卡片网格（容器查询自适应列数）、hover 放大、详情弹窗、快速加购、库存状态标签；中栏三态明确（**搜索结果** / **搜索无结果**：`没有找到符合条件的商品` + 放宽条件建议 + 仍给 12 件精选 / **为你推荐**），副标题写明展示口径（「共 N 件 · 展示前 M 件」，命中不多时只写件数），与「数据快照」面板里的**目录总量**区分开；**命中多于 12 件时副标题旁出现「查看全部 N 件」** → 打开浏览视图看全（无筛选条件时直接由目录现算，见下一行） |
...
![中栏浏览视图：点品类 chip（或「查看全部 N 件」）直接看全量商品——数据走客户端目录的按需 chunk，零 LLM 依赖，可翻到底、可排序、ESC 退出](docs/rehearsal/round3-browse-all.jpg)
...
| `searchResultIds` | 覆盖 | 本轮命中商品的前 `SEARCH_RESULT_ID_LIMIT = 120` 个 id（「查看全部 N 件」的数据源）；仅在**有有效筛选条件且命中多于展示**时写入，快照侧按意图门挡（只有 search / refine 轮下发） |
...
7. ~~**分类浏览入口未实现**~~：**已实现（2026-09-26 起改为直接打开浏览视图）** —— 中栏顶部「按品类浏览」chip 行（品类 + 件数）点击即打开该品类的**全量浏览视图**（客户端目录按需 chunk 现算，零 LLM 依赖、不进对话、不带残留条件）；带条件的品类检索仍可用对话「帮我看看<品类>的商品」。同一视图还承载「查看全部 N 件」（本轮命中 id 列表）与「看全部 420 件」（客户端目录全量）。
